gseaplot2_none <-
            ...
            ES_geom = "line")
  {
    ...
    p <-
      ggplot(gsdata, aes_(x = ~ x)) + xlab(NULL) + theme_classic(base_size) +
      theme(
    ...
    p2 <-
      ggplot(gsdata, aes_(x = ~ x)) + geom_linerange(aes_(
        ymin = ~ ymin,
        ymax = ~ ymax,
        color = ~ Description
      )) + xlab(NULL) + ylab(NULL) +
      theme_classic(base_size) + theme(
        legend.position = "none",
    ...
    p.pos <- p + geom_segment(data = df2,
                              ...
                              color = "grey")
    p.pos <- p.pos + ylab(NULL) + xlab(NULL) +
      theme(
        axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        plot.margin = margin(
    ...
    if (pvalue_table) {
      ...
      p.res <-
        p.res + theme(legend.position = "none") + annotation_custom(
          ...
          xmax = quantile(p.res$data$x,
                          0.95),
          ymin = quantile(p.res$data$runningScore,
                          0.75),
          ymax = quantile(p.res$data$runningScore,
                          0.9)
        )
    }
