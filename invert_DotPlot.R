          ...
          scale = TRUE, scale.by = "radius", scale.min = NA, scale.max = NA) 
{
    ...
    plot <- ggplot(data = data.plot, mapping = aes_string(x = "id", #x和y的值互换
                                                          y = "features.plot")) + geom_point(mapping = aes_string(size = "pct.exp", 
        ...
        labs(x = ifelse(test = is.null(x = split.by), 
                        yes = "Identity", no = "Split Identity"), y = "Features") + theme_cowplot()
    if (!is.null(x = feature.groups)) {
